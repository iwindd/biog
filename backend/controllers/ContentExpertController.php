...
use backend\models\ContentExpertSearch;
...
/**
 * ContentExpertController implements the CRUD actions for Content model.
 */
class ContentExpertController extends Controller
{
    ...
    {
        $searchModel = new ContentExpertSearch();
    ...
    {
       $modelContent=ContentExpert::find()->where(['content_id'=>$id])->one();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelExpert->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                Upload::handleFileCenterPicture($model, 'content-expert', $case_error);
                ...
                if ($model->save()) {
                    ...
                    Upload::handleFileCenterGallery($model, 'content-expert', $case_error, $checkUpdate);
                    ...
                    $modelExpert->content_id = $model->id;
                    ...
                    if ($modelExpert->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $model->id, 'create content expert', 'เพิ่มเนื้อหาภูมิปัญญา/ปราชญ์');
    ...
    public function actionUpdate($id)
    {
        ...
        $modelExpertOld = ContentExpert::find()->where(['content_id' => $id])->one();
        $modelExpert = new ContentExpert();
        ...
        if ($model->load(Yii::$app->request->post()) && $modelExpert->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                $mainPicture = Upload::uploadPictureNoPermission($model, 'content-expert', $modelOld->getOldAttribute('picture_path'), $del, 'picture_path');
                ...
                if ($model->save()) {
                    ...
                    // upload multiple image
                    $files = Upload::uploadsNoPermimission($model, 'content-expert');
                    if (!empty($files)) {
                    ...
                    if ($modelExpert->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            if ($model->status == 'rejected') {
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content expert', 'แก้ไขเนื้อหาภูมิปัญญา/ปราชญ์ สถานะไม่อนุมัติหมายเหตุ: '.$model->note);
                            }else{
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content expert', 'แก้ไขเนื้อหาภูมิปัญญา/ปราชญ์');
                            }
    ...
    public function actionDelete($id)
    {
        ...
        if ($content['status'] == 'success') {
            ...
            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $id, 'delete content expert', 'ลบเนื้อหาภูมิปัญญา/ปราชญ์');
    ...
    public function actionStartExport()
    {
        ...
        $filters = $this->getExpertExportFilters();
        $filters['date_from'] = $dateFrom;
        $filters['date_to'] = $dateTo;

        $exportJob = new \common\jobs\ExportJob([
            'typeKey' => 'content_expert',
            'filters' => $filters,
            'userId' => Yii::$app->user->identity->id,
            'baseFileName' => 'รายงานข้อมูลภูมิปัญญา',
        ]);
        ...
        $placeholderJob = $service->createJob('content_expert', $filters, $userId, $userEmail);
        ...
        if ($job) {
            ...
            return [
                ...
                'job' => [
                    ...
                    'downloadUrl' => $job['download_ready'] ? Yii::$app->urlManager->createUrl(['/content-expert/download-export', 'jobId' => $job['id']]) : '',
                ],
            ];
        }
    ...
    private function getExpertExportFilters()
    {
        $searchFilters = Yii::$app->request->get('ContentExpertSearch', []);
