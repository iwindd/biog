...
use backend\models\ContentFungiSearch;
...
/**
 * ContentFungiController implements the CRUD actions for Content model.
 */
class ContentFungiController extends Controller
{
    ...
    {
        $searchModel = new ContentFungiSearch();
    ...
    {
       $modelContent=ContentFungi::find()->where(['content_id'=>$id])->one();
                ...
                if ($model->save()) {
                    ...
                    if ($modelFungi->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $model->id, 'create content fungi', 'เพิ่มเนื้อหาจุลินทรีย์');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelFungi->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                $mainPicture = Upload::uploadPictureNoPermission($model, 'content-fungi', $modelOld->getOldAttribute('picture_path'), $del, 'picture_path');
                    ...
                    if ($modelFungi->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            if ($model->status == 'rejected') {
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content fungi', 'แก้ไขเนื้อหาจุลินทรีย์ สถานะไม่อนุมัติหมายเหตุ: '.$model->note);
                            }else{
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content fungi', 'แก้ไขเนื้อหาจุลินทรีย์');
                            }
    ...
    public function actionDelete($id)
    {
        ...
        if ($content['status'] == 'success') {
            ...
            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $id, 'delete content fungi', 'ลบเนื้อหาจุลินทรีย์');
    ...
    public function actionStartExport()
    {
        ...
        $filters = $this->getFungiExportFilters();
        $filters['date_from'] = $dateFrom;
        $filters['date_to'] = $dateTo;

        $exportJob = new \common\jobs\ExportJob([
            'typeKey' => 'content_fungi',
            'filters' => $filters,
            'userId' => Yii::$app->user->identity->id,
            'baseFileName' => 'รายงานข้อมูลจุลินทรีย์',
        ]);
        ...
        $placeholderJob = $service->createJob('content_fungi', $filters, $userId, $userEmail);
        ...
        if ($job) {
            ...
            return [
                ...
                'job' => [
                    ...
                    'downloadUrl' => $job['download_ready'] ? Yii::$app->urlManager->createUrl(['/content-fungi/download-export', 'jobId' => $job['id']]) : '',
                ],
            ];
        }
    ...
    private function getFungiExportFilters()
    {
        $searchFilters = Yii::$app->request->get('ContentFungiSearch', []);
