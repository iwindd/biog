...
use backend\models\ContentEcotourismSearch;
...
/**
 * ContentEcotourismController implements the CRUD actions for Content model.
 */
class ContentEcotourismController extends Controller
{
    ...
    {
        $searchModel = new ContentEcotourismSearch();
        $searchModel->type_id = 5;
    ...
    {
       $modelContent=ContentEcotourism::find()->where(['content_id'=>$id])->one();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelEcotourism->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                Upload::handleFileCenterPicture($model, 'content-ecotourism', $case_error);
                ...
                if ($model->save()) {
                    ...
                    Upload::handleFileCenterGallery($model, 'content-ecotourism', $case_error, $checkUpdate);
                    ...
                    if ($modelEcotourism->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $model->id, 'create content ecotourism', 'เพิ่มเนื้อหาแหล่งท่องเที่ยวเชิงนิเวศ');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelEcotourism->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                $mainPicture = Upload::uploadPictureNoPermission($model, 'content-ecotourism', $modelOld->getOldAttribute('picture_path'), $del, 'picture_path');
                ...
                if ($model->save()) {
                    ...
                    // upload multiple image
                    $files = Upload::uploadsNoPermimission($model, 'content-ecotourism');
                    if (!empty($files)) {
                    ...
                    if ($modelEcotourism->save()) {
                        ...
                        if ($checkUpdate) {
                            ...
                            if ($model->status == 'rejected') {
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content ecotourism', 'แก้ไขเนื้อหาแหล่งท่องเที่ยวเชิงนิเวศ สถานะไม่อนุมัติหมายเหตุ: '.$model->note);
                            }else{
                                BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $newContentId, 'update content ecotourism', 'แก้ไขเนื้อหาแหล่งท่องเที่ยวเชิงนิเวศ');
                            }
    ...
    public function actionDelete($id)
    {
        ...
        if ($content['status'] == 'success') {
            ...
            BackendHelper::saveUserLog('content', Yii::$app->user->identity->id, $id, 'delete content ecotourism', 'ลบเนื้อหาแหล่งท่องเที่ยวเชิงนิเวศ');
