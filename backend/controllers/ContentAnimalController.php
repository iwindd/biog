...
class ContentAnimalController extends Controller
{
    ...
    public function actionStartExport()
    {
        ...
        $filters = $this->getAnimalExportFilters();
        $filters['date_from'] = $dateFrom;
        $filters['date_to'] = $dateTo;

        $exportJob = new \common\jobs\ExportJob([
            'typeKey' => 'content_animal',
            'filters' => $filters,
            'userId' => Yii::$app->user->identity->id,
            'baseFileName' => 'รายงานข้อมูลสัตว์',
        ]);
        ...
        $placeholderJob = $service->createJob('content_animal', $filters, $userId, $userEmail);
        ...
        if ($job) {
            ...
            return [
                ...
                'job' => [
                    ...
                    'downloadUrl' => $job['download_ready'] ? Yii::$app->urlManager->createUrl(['/content-animal/download-export', 'jobId' => $job['id']]) : '',
                ],
            ];
        }
    ...
    private function getAnimalExportFilters()
    {
        $searchFilters = Yii::$app->request->get('ContentAnimalSearch', []);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelAnimal->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                $post = Yii::$app->request->post();
                Upload::handleFileCenterPicture($model, 'content-animal', $case_error);
                ...
                if ($model->save()) {
                    ...
                    Upload::handleFileCenterGallery($model, 'content-animal', $case_error, $checkUpdate);
