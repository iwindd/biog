...
for($tab = 0; $tab < 1; $tab++ ){
    ...
    $sheet->setCellValue('A1', 'ข้อมูลภูมิปัญญา/ปราชญ์ทั้งหมด');
    ...
    $sheet->setCellValue('B2', 'ชื่อ ภูมิปัญญา/ปราชญ์');
    ...
    $sheet->setCellValue('D2', 'วัน/เดือน/ปีเกิด ของผู้รู้/ปราชญ์');
    $sheet->getStyle('D2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('E2', 'ภูมิปัญญาที่เชี่ยวชาญ');
    $sheet->getStyle('E2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('F2', 'หมวดหมู่ภูมิปัญญา');
    $sheet->getStyle('F2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('G2', 'อาชีพ ผู้รู้/ปราชญ์');
    $sheet->getStyle('G2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('H2', 'เลขบัตรประชาชน ผู้รู้/ปราชญ์');
    $sheet->getStyle('H2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('I2', 'โทรศัพท์ ผู้รู้/ปราชญ์');
    ...
    if(!empty($model)){
        ...
        foreach ($model as $keyPlan => $value){
            ...
            $sheet->setCellValue('B'.$i, $value['expert_firstname']." ".$value['expert_lastname']);
    ...
    $sheet->setTitle("ภูมิปัญญา_ปราชญ์");
...
$file_name = "รายงานข้อมูลภูมิปัญญา_ปราชญ์_".$date."_".$h."_".$i."_".$s."s";
//
