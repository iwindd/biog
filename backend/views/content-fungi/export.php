...
for($tab = 0; $tab < 1; $tab++ ){
    ...
    $sheet->setCellValue('A1', 'ข้อมูลจุลินทรีย์ทั้งหมด');
    ...
    $sheet->setCellValue('B2', 'ชื่ออื่น');
    ...
    if(!empty($model)){
        ...
        foreach ($model as $keyPlan => $value){
            ...
            $sheet->setCellValue('B'.$i, $value['other_name']);
            ...
            $sheet->setCellValue('D'.$i,  str_replace('&nbsp;', '', strip_tags($value['features'])));
            $sheet->getStyle('D'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);;

            $sheet->setCellValue('E'.$i,  str_replace('&nbsp;', '', strip_tags($value['benefit'])));
            $sheet->getStyle('E'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);;
              
            $sheet->setCellValue('F'.$i, $value['found_source']);
            $sheet->getStyle('F'.$i)->applyFromArray($styleLeft);   

            $sheet->setCellValue('G'.$i, $value['common_name']);
            $sheet->getStyle('G'.$i)->applyFromArray($styleLeft);   
               
            $sheet->setCellValue('H'.$i, $value['scientific_name'] );
            $sheet->getStyle('H'.$i)->applyFromArray($styleLeft); 

            $sheet->setCellValue('I'.$i, $value['family_name']  );
            $sheet->getStyle('I'.$i)->applyFromArray($styleLeft); 
    ...
    $sheet->setTitle("รายงานข้อมูลจุลินทรีย์");
...
$file_name = "รายงานข้อมูลจุลินทรีย์_".$date."_".$h."_".$i."_".$s."s";
//
