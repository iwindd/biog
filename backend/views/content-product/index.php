...
/* @var $searchModel backend\models\ContentProductSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'จัดการข้อมูลผลิตภัณฑ์';
$this->params['breadcrumbs'][] = $this->title;
$category =  ArrayHelper::map(ProductCategory::find()->all(), 'id', 'name');
$url = "";
if(!empty($_GET['ContentProductSearch'])){
    if(!empty($_GET['ContentProductSearch']['name'])){
        $url = "?name=".$_GET['ContentProductSearch']['name'];
    }

    if(!empty($_GET['ContentProductSearch']['created_by_user_id'])){
        if (!empty($url)) {
            $url = $url."&created_by_user_id=".$_GET['ContentProductSearch']['created_by_user_id'];
        }else{
            $url = "?created_by_user_id=".$_GET['ContentProductSearch']['created_by_user_id'];
        }
    }

    if(!empty($_GET['ContentProductSearch']['updated_by_user_id'])){
        if (!empty($url)) {
            $url = $url."&updated_by_user_id=".$_GET['ContentProductSearch']['updated_by_user_id'];
        }else{
            $url = "?updated_by_user_id=".$_GET['ContentProductSearch']['updated_by_user_id'];
        }
    }



    if(!empty($_GET['ContentProductSearch']['approved_by_user_id'])){
        if (!empty($url)) {
            $url = $url."&approved_by_user_id=".$_GET['ContentProductSearch']['approved_by_user_id'];
        }else{
            $url = "?approved_by_user_id=".$_GET['ContentProductSearch']['approved_by_user_id'];
        }
    }

    if(!empty($_GET['ContentProductSearch']['note'])){
        if (!empty($url)) {
            $url = $url."&note=".$_GET['ContentProductSearch']['note'];
        }else{
            $url = "?note=".$_GET['ContentProductSearch']['note'];
        }
    }

    if(!empty($_GET['ContentProductSearch']['status'])){
        if (!empty($url)) {
            $url = $url."&status=".$_GET['ContentProductSearch']['status'];
        }else{
            $url = "?status=".$_GET['ContentProductSearch']['status'];
        }
    }

    if(!empty($_GET['ContentProductSearch']['updated_at'])){
        if (!empty($url)) {
            $url = $url."&updated_at=".$_GET['ContentProductSearch']['updated_at'];
        }else{
            $url = "?updated_at=".$_GET['ContentProductSearch']['updated_at'];
        }
    }
        ...
        'columns' => [
            ...
            [
                ...
                'value'=>function($model){
                    $image = Upload::readfilePictureNoPermission('content-product',$model->picture_path);
                    if(empty($image)){
                        return '<img width="100%" src="/admin/images/BIOG_default_product.png" />';
                    }
                    return $image;
                }
            ],
