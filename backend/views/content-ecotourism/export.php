...
for($tab = 0; $tab < 1; $tab++ ){
    ...
    $sheet->setCellValue('A1', 'ข้อมูลแหล่งท่องเที่ยวเชิงนิเวศทั้งหมด');

    $sheet->getStyle('A1:S1')->applyFromArray($styleContentHeader);
    $sheet->mergeCells('A1:S1');

    $sheet->setCellValue('A2', 'ชื่อสถานที่ท่องเที่ยว');
    $sheet->getStyle('A2')->applyFromArray($styleContentHeader);

    $sheet->setCellValue('B2', 'รายละเอียด');
    ...
    if(!empty($model)){
        ...
        foreach ($model as $keyPlan => $value){
            ...
            $sheet->setCellValue('B'.$i,  str_replace('&nbsp;', '', strip_tags($value['description'])));
            ...
            $sheet->setCellValue('D'.$i,  str_replace('&nbsp;', '', strip_tags($value['contact_name'])));
            $sheet->getStyle('D'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);

            $sheet->setCellValue('E'.$i,  str_replace('&nbsp;', '', strip_tags($value['contact'])));
            $sheet->getStyle('E'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);
              
            $sheet->setCellValue('F'.$i, $value['phone']);
            $sheet->getStyle('F'.$i)->applyFromArray($styleLeft);   

            $sheet->setCellValue('G'.$i,  str_replace('&nbsp;', '', strip_tags($value['other_information'])));
            $sheet->getStyle('G'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);   
               
            $sheet->setCellValue('H'.$i,  str_replace('&nbsp;', '', strip_tags($value['travel_information'] )));
            $sheet->getStyle('H'.$i)->applyFromArray($styleLeft)->getAlignment()->setWrapText(true);

            $sheet->setCellValue('I'.$i, $value['address']  );
    ...
    $sheet->setTitle("แหล่งท่องเที่ยวเชิงนิเวศ");
...
$file_name = "รายงานข้อมูลแหล่งท่องเที่ยวเชิงนิเวศ_".$date."_".$h."_".$i."_".$s."s";
//
