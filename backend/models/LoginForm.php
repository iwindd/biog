...
class LoginForm extends Model
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'requiredFields' => [['login'], 'required','message' => "Username cannot be blank."], 
        ...
        if (!$this->module->debug) {
            $rules = array_merge($rules, [
                'requiredFields' => 
                    [['login'], 'required' ,'message' => "Username cannot be blank."],
                    [['password'], 'required' ,'message' => '{attribute} cannot be empty'],

                'passwordValidate' => [
                    'password',
                    function ($attribute) {
                        if ($this->user === null || !Password::validate($this->password, $this->user->password_hash)) {
                            $this->addError($attribute, Yii::t('app', 'Username or Password Invalid.'));
                        }
                    }
                ]
            ]);
