                ...
                function ($attribute) {
                    if ($this->user !== null) {
                        ...
                        if ($this->user->getIsBlocked()) {
                            $this->addError($attribute, Yii::t('user', 'บัญชีของคุณถูกบล็อกกรุณาติดต่อเจ้าหน้าที่'));
                        }
                    }
                }
            ],
        ...
        if (!$this->module->debug) {
            $rules = array_merge($rules, [
                ...
                'loginValidate' => [
                    ...
                    function ($attribute) {
                        if ($this->user === null) {
                            $this->addError($attribute, Yii::t('app', 'อีเมลไม่ถูกต้อง'));
                        }
                    }
                ],
                'passwordValidate' => [
                    ...
                    function ($attribute) {
                        if ($this->user === null || !Password::validate($this->password, $this->user->password_hash)) {
                            $this->addError($attribute, Yii::t('app', 'รหัสผ่านไม่ถูกต้อง'));
                        }
                    }
                ]
            ]);
